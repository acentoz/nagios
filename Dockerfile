...
RUN apt-get update && \
  apt-get install -y --no-install-recommends \
    ...
    m4 mysql-client \
    netcat \
    python-pip python-setuptools python-dev python3-dev python3-setuptools parallel php7.0-xml postfix \
    runit \
...
RUN sudo python -m pip install --upgrade --force setuptools
RUN sudo python -m pip install --upgrade --force pip
...
ADD config/nagios.cfg /usr/local/nagios/etc/nagios.cfg
ADD config/windows.cfg /usr/local/nagios/etc/objects/windows.cfg
